...
export async function scrapeTeamPlayers(cfg?: CategoryConfig) {
  ...
  try {
    ...
    try {
      await page.goto(config.players_url, { waitUntil: "domcontentloaded", timeout: 90000 });
    } catch {
    ...
    try {
      await page.waitForSelector("#team_squad .staff, #team_squad .innerbox", { timeout: 30000 });
    } catch {
    ...
    const squadBox = $("#team_squad .innerbox").length ? $("#team_squad .innerbox") : null;
    ...
    if (squadBox && squadBox.length > 0) {
      squadBox.each((_, box) => {
        ...
        $(box).find(".staff").each((_, el) => {
          ...
          if (name && externalId) players.push({ externalId, name, number, position, age, photoUrl });
        });
      });
    } else {
      $("a[href*='/jogador/']").each((_, el) => {
        ...
        if (name && externalId && !players.find((p) => p.externalId === externalId)) {
          ...
          players.push({ externalId, name, number, position: "Unknown", age: null, photoUrl });
        }
      });
    }

    // Staff técnico
    const staffBox = $("#team_staff .innerbox").length ? $("#team_staff .innerbox") : null;
    if (staffBox) {
      staffBox.each((_, box) => {
        ...
        $(box).find(".staff").each((_, el) => {
          ...
          const age = parseAge($(el), $);
          const photoUrl = parsePhoto($(el), $);
          if (name && externalId) players.push({ externalId, name, number: null, position: sectionRole, age, photoUrl });
        });
      });
    }
...
export async function savePlayersAndSquad(players: any[], category: string = "over19") {
